...
File file = new File("Output/exchangeRate.csv")

if (!file.exists()) {
	throw new Exception("CSV file not found: " + "Output/exchangeRate.csv")
}

List<String> lines = file.readLines("UTF-8")

String[] tokens = lines[1].split(",")
BigDecimal usd = new BigDecimal(tokens[0])
BigDecimal vnd = new BigDecimal(tokens[1])
BigDecimal exchangeRate = vnd.divide(usd)

List<Employee> employees = EmployeeReader.readEmployeesFromCsv("Output/employees.csv")
